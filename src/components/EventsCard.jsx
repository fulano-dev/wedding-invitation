    ...
    <AnimatePresence>
      {isOpen && (
        <>
          ...
          >
            <div className="bg-neutral-900 transform -translate-x-1/2 -translate-y-1/2 rounded-2xl p-6 shadow-2xl border border-yellow-900">
              {children}
            </div>
          </motion.div>
        </>
...
const CalendarButton = ({ icon: Icon, label, onClick, className = "" }) => (
  <motion.button
    onClick={onClick}
  className={`flex items-center space-x-3 w-full p-4 rounded-xl border border-yellow-900 hover:bg-yellow-900/30 transition-colors ${className}`}
...
END:VCALENDAR`;
  ...
  return (
    <div className="relative">
      <motion.div
  className="bg-neutral-900 rounded-2xl p-6 shadow-sm border border-yellow-900 space-y-4"
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.5 }}
      >
        <div className="flex justify-between items-center">
          <h3 className="text-xl font-semibold text-neutral-100">{eventData.title.split(' - ')[0]}</h3>
          <motion.button
  ...
  <div className="space-y-3 text-neutral-100">
      ...
      >
        <div className="space-y-6 ">
          <div className="flex justify-between  items-center">
            <h3 className="text-xl font-semibold text-yellow-400">Adicionar ao Calendário</h3>
            <motion.button
          ...
          <div className="space-y-3">
            <CalendarButton
              icon={(props) => <Chrome {...props} className="w-5 h-5 text-yellow-600" />}
              label="Google Calendar"
              onClick={() => window.open(googleCalendarLink(), '_blank')}
            />

            <CalendarButton
              icon={(props) => <Apple {...props} className="w-5 h-5 text-yellow-400" />}
